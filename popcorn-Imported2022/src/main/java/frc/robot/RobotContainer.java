...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    m_intake.setDefaultCommand(new RunCommand(() -> {
      var speed = Math.abs(operatorController.getRightY()) > Constants.kJoystickTurnDeadzone
        ? operatorController.getRightY()
        : 0; 
      m_intake.set(speed);
    }, m_intake));
    m_serializer.setDefaultCommand(new RunCommand(() -> {
      var speed = Math.abs(operatorController.getLeftY()) > Constants.kJoystickTurnDeadzone
